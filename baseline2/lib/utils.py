...
def compute_metrics(p: EvalPrediction):
    """Compute F1 score for each task and return the average F1."""
    # If multi-task, p.predictions is (num_samples, num_tasks, num_classes)
    # p.label_ids is (num_samples, num_tasks)
    preds = p.predictions
    labels = p.label_ids
    if preds.ndim == 3:
        # Multi-task: shape (num_samples, num_tasks, num_classes)
        num_tasks = preds.shape[1]
        f1_list = []
        for i in range(num_tasks):
            task_preds = np.argmax(preds[:, i, :], axis=1)
            task_labels = labels[:, i]
            f1 = f1_score(task_labels, task_preds, pos_label=1)
            f1_list.append(f1)
        avg_f1 = np.mean(f1_list)
        return {f"f1_task{i}": f1 for i, f1 in enumerate(f1_list)} | {"f1_avg": avg_f1}
    else:
        # Single task: shape (num_samples, num_classes)
